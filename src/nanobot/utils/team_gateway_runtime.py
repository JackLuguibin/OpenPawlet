...
def active_team_gateway_path(workspace: Path) -> Path:
    return workspace / ".nanobot_console" / ACTIVE_TEAM_GATEWAY_FILE
...
def resolve_gateway_team_context(
    ...
    Precedence:
    1. ``NANOBOT_TEAM_ID`` + ``NANOBOT_TEAM_ROOM_ID`` when both set (legacy override).
    2. ``active_team_gateway.json`` + ``teams.json`` when valid.
    """
    env_tid = os.environ.get("NANOBOT_TEAM_ID", "").strip()
    env_rid = os.environ.get("NANOBOT_TEAM_ROOM_ID", "").strip()
...
def _members_from_env_or_teams(workspace: Path, team_id: str) -> list[str]:
    csv = os.environ.get("NANOBOT_TEAM_MEMBER_IDS", "").strip()
    if csv:
...
def resolve_effective_gateway_agent_id(workspace: Path) -> str | None:
    """Resolve logical agent_id when :envvar:`NANOBOT_AGENT_ID` is unset.
    ...
    :class:`~nanobot.agent.loop.AgentLoop`’s default identity.
    """
