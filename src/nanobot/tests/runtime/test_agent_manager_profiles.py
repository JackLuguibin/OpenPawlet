...
def _fake_embedded(
    ...
    *,
    ...
    """Minimal stand-in for ``EmbeddedNanobot`` used by ``UnifiedAgentManager``."""
...
# ---------------------------------------------------------------------------
# EmbeddedNanobot._list_enabled_standalone_agents — selection rules
# ---------------------------------------------------------------------------
...
def test_list_enabled_standalone_agents_skips_disabled_and_team_members(
    ...
    result = EmbeddedNanobot._list_enabled_standalone_agents(embedded)  # type: ignore[arg-type]
...
def test_profile_fingerprint_changes_when_runtime_fields_change(
    ...
    fp_before = EmbeddedNanobot._profile_fingerprint(embedded, base)  # type: ignore[arg-type]

    # Same content -> same fingerprint.
    same = AgentProfile(id="x", name="X-renamed", model="m1", system_prompt="hi")
    fp_same = EmbeddedNanobot._profile_fingerprint(embedded, same)  # type: ignore[arg-type]
    ...
    assert EmbeddedNanobot._profile_fingerprint(embedded, changed_model) != fp_before  # type: ignore[arg-type]

    changed_prompt = AgentProfile(
        id="x", name="X", model="m1", system_prompt="bye"
    )
    assert EmbeddedNanobot._profile_fingerprint(embedded, changed_prompt) != fp_before  # type: ignore[arg-type]

    changed_skills = AgentProfile(
        id="x", name="X", model="m1", system_prompt="hi", skills=["alpha"]
    )
    assert EmbeddedNanobot._profile_fingerprint(embedded, changed_skills) != fp_before  # type: ignore[arg-type]
