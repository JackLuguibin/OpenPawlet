...
4. ``~/.nanobot/nanobot_web.json`` under the ``server`` key
...
def isolated_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    fake_nanobot_config = tmp_path / "config.json"
    monkeypatch.setattr(nanobot_loader, "_current_config_path", fake_nanobot_config)
    # Clear every NANOBOT_SERVER_* env var so tests control them explicitly.
    for key in list(__import__("os").environ):
        if key.startswith("NANOBOT_SERVER_"):
            monkeypatch.delenv(key, raising=False)
    return tmp_path / "nanobot_web.json"
...
def test_reset_settings_cache_picks_up_new_env(
    ...
    monkeypatch.setenv("NANOBOT_SERVER_PORT", "9400")
    reset_settings_cache()
    second = get_settings()
