...
import { parseNanobotStatusJson } from "./statusParse";
import type { NanobotContextUsage } from "./types";

/**
 * Inputs the chat page must hand over so the hook can track the nanobot
 * `/status-json` lifecycle without re-implementing WebSocket plumbing.
 */
interface UseNanobotContextUsageParams {
  useNanobotChannel: boolean;
  nanobotWsReady: boolean;
  currentBotId: string | null;
  sendNanobotMessage: (payload: { content: string; botId: string | null }) => void;
  /**
 ...
 */
export function useNanobotContextUsage({
  useNanobotChannel,
  nanobotWsReady,
  currentBotId,
  sendNanobotMessage,
  activeSessionKey,
}: UseNanobotContextUsageParams): UseNanobotContextUsageResult {
