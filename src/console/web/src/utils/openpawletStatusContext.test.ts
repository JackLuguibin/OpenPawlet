...
} from "./openpawletStatusContext";

describe("openpawletStatusContext", () => {
  it("returns null when no context is present", () => {
    expect(extractOpenPawletStatusContext({})).toBeNull();
  ...
  it("detects the direct `context` shape", () => {
    ...
    expect(extractOpenPawletStatusContext({ context: ctx })).toEqual(ctx);
  ...
  it("detects the wrapped `data.context` shape", () => {
    ...
    expect(extractOpenPawletStatusContext(payload)).toEqual(ctx);
    expect(outboundDataHasStatusContext(payload)).toBe(true);
  });

  it("ignores arrays in `data`", () => {
    expect(extractOpenPawletStatusContext({ data: [1, 2, 3] })).toBeNull();
  ...
  it("ignores primitive `context` values", () => {
    ...
    expect(extractOpenPawletStatusContext({ context: "nope" })).toBeNull();
