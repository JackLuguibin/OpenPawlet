/**
 * Nanobot "status context" payload shape checks.
 *
 * Nanobot's outbound `message.data` can carry a JSON document whose `context`
 * key (or nested `data.context`) holds `tokens_estimate` / `window_total`
 * / `percent_used`. Both the chat page renderer and the `/nanobot-ws` frame
 * mapper need to detect this shape:
 *   - `useNanobotChannelWebSocket.ts` synthesizes a `nanobot_status_json`
...
export function outboundDataHasStatusContext(
  ...
  return extractNanobotStatusContext(root) !== null;
}

/** Pull out the `context` sub-object from an outbound status payload, if any. */
export function extractNanobotStatusContext(
  root: Record<string, unknown>,
