...
- Publish to OpenPawlet bus via BaseChannel._handle_message()
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # Disable botpy's file log — OpenPawlet uses loguru; default "botpy.log" fails on read-only fs
...
class QQConfig(Base):
    ...
    # Optional: directory to save inbound attachments. If empty, use OpenPawlet get_media_dir("qq").
    media_dir: str = ""
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.home() / ".openpawlet" / "media" / "qq"
        else:
            root = Path.home() / ".openpawlet" / "media" / "qq"
