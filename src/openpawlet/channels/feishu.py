...
class FeishuChannel(BaseChannel):
    ...
    async def send_delta(
        ...
        Supported metadata keys:
            ...
            _resuming:   When true, skip reaction cleanup (multi-round agent turn).
            chat_type:   "group" or "p2p" — controls reply-in-thread for streaming cards.
            reaction_id: Optional explicit id (otherwise looked up from ``_reaction_ids``).
        """
        if not self._client:
            return
        meta = metadata or {}
        ...
        if meta.get("_stream_end"):
            ...
            if message_id and not meta.get("_resuming"):
                reaction_cleared = self._reaction_ids.pop(message_id, None) or meta.get(
                    "reaction_id"
                )
                if reaction_cleared:
                    await self._remove_reaction(message_id, reaction_cleared)
            ...
            for chunk in self._split_elements_by_table_limit(
                ...
                if fallback_msg_id:
                    await loop.run_in_executor(
                        None,
                        lambda fm=fallback_msg_id, c=card, md=dict(meta): self._reply_message_sync(
                            fm,
                            "interactive",
                            c,
                            reply_in_thread=self._should_use_reply_in_thread(md),
                        ),
                    )
                else:
                    await loop.run_in_executor(
        ...
        try:
            ...
            if msg.metadata.get("_tool_hint"):
                ...
                if _th_msg_id:
                    await loop.run_in_executor(
                        None,
                        lambda tm=_th_msg_id, md=dict(msg.metadata): self._reply_message_sync(
                            tm,
                            "interactive",
                            card,
                            reply_in_thread=self._should_use_reply_in_thread(md),
