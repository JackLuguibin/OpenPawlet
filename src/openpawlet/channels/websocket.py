"""WebSocket server channel: OpenPawlet acts as a WebSocket server and serves connected clients."""
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      OpenPawlet and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-OpenPawlet-Auth: <secret>``.
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = headers.get("X-OpenPawlet-Auth") or headers.get("x-openpawlet-auth")
    if not header_token:
        return False
...
class WebSocketChannel(BaseChannel):
    ...
    def set_session_busy_resolver(self, fn: Callable[[str], bool] | None) -> None:
        """Register whether an OpenPawlet session key is mid-turn (gateway + AgentLoop only)."""
