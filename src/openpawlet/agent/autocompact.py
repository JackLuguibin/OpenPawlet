...
from openpawlet.session.manager import Session, SessionManager, _as_agent_aware
from openpawlet.utils.helpers import local_now

if TYPE_CHECKING:
    from openpawlet.agent.memory import Consolidator
...
class AutoCompact:
    ...
    def _is_expired(self, ts: datetime | str | None, now: datetime | None = None) -> bool:
        ...
        if isinstance(ts, str):
            ts = _as_agent_aware(datetime.fromisoformat(ts.replace("Z", "+00:00")), tz)
        elif isinstance(ts, datetime):
            ts = _as_agent_aware(ts, tz)
    ...
    def _split_unconsolidated(
        ...
        probe = Session(
            ...
            metadata={},
            last_consolidated=0,
            agent_timezone=session.agent_timezone,
        )
    ...
    def prepare_session(self, session: Session, key: str) -> tuple[Session, str | None]:
        ...
        if "_last_summary" in session.metadata:
            ...
            return session, self._format_summary(
                meta["text"],
                _as_agent_aware(
                    datetime.fromisoformat(meta["last_active"].replace("Z", "+00:00")),
                    self.sessions.agent_timezone,
                ),
            )
        return session, None
