...
def load_console_agent_row(workspace: Path, agent_id: str) -> dict[str, Any] | None:
    ...
    2. ``<workspace>/agents/<id>.json`` — legacy single-file Console
       record.
    3. ``.openpawlet_console/agents.json`` ``agents`` array (very old).
    """
    ...
    if per.is_file():
        ...
        if isinstance(raw, dict):
            return raw
    path = workspace_console_subdir(workspace) / "agents.json"
    if not path.is_file():
        return None
    try:
...
def load_console_team_row(workspace: Path, team_id: str) -> dict[str, Any] | None:
    """Return team dict from ``.openpawlet_console/teams.json`` for *team_id*, or None."""
...
def resolve_gateway_identity_overrides(
    ...
    *,
    ...
    if not _tid:
        _tid = os.environ.get("OPENPAWLET_TEAM_ID", "").strip() or None
    if _tid:
