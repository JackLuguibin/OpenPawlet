...
OpenPawlet can act as a WebSocket server, allowing external clients (web apps, CLIs, scripts) to interact with the agent in real time via persistent connections.
...
- Session resume — clients can pass `chat_id` (UUID) on connect to resume a persisted conversation; a second connection with the same id replaces the first
...
async def main():
    async with websockets.connect("ws://127.0.0.1:8765/?client_id=alice") as ws:
        ...
        await ws.send(json.dumps({"content": "Hello OpenPawlet!"}))
...
1. Client sends `GET {tokenIssuePath}` with `Authorization: Bearer {tokenIssueSecret}` (or `X-OpenPawlet-Auth` header).
...
`chat_id` is a *capability*: anyone holding a valid WebSocket auth credential and the chat_id can attach to that conversation and see its output. This matches OpenPawlet's local, single-user default. Multi-tenant deployments should namespace chat_ids per user (or introduce a per-tenant auth gate) — OpenPawlet does not enforce multi-tenancy on its own.
...
- An HTTP file server serving the OpenPawlet media directory
