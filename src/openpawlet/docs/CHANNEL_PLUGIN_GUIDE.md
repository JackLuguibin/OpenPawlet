...
Build a custom OpenPawlet channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of OpenPawlet (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.
...
```
openpawlet-channel-webhook/
├── openpawlet_channel_webhook/
...
```python
# openpawlet_channel_webhook/__init__.py
from openpawlet_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# openpawlet_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "openpawlet-channel-webhook"
version = "0.1.0"
dependencies = ["open-pawlet", "aiohttp"]

[project.entry-points."openpawlet.channels"]
webhook = "openpawlet_channel_webhook:WebhookChannel"
...
packages = ["openpawlet_channel_webhook"]
```
...
Built-in channels use Pydantic config models (subclassing `Base` from `openpawlet.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `openpawlet.config.schema.Base`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `openpawlet-channel-{name}` | `openpawlet-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `openpawlet_channel_{name}` | `openpawlet_channel_webhook` |
