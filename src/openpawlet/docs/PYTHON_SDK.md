...
Use OpenPawlet programmatically — load config, run the agent, get results.

## Quick Start

```python
import asyncio
from openpawlet import OpenPawlet

async def main():
    bot = OpenPawlet.from_config()
...
### `OpenPawlet.from_config(config_path?, *, workspace?)`

Create an `OpenPawlet` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.openpawlet/config.json` (with compatibility fallback for older default locations). |
...
```python
from openpawlet.agent import AgentHook, AgentHookContext
...
async def main():
    bot = OpenPawlet.from_config(workspace="/my/project")
