...
def get_config_path() -> Path:
    ...
    return Path.home() / ".openpawlet" / "config.json"
...
def resolve_config_env_vars(config: Config) -> Config:
    ...
    Walks via :func:`_resolve_in_place` so fields declared with ``exclude=True``
    (e.g. ``DreamConfig.cron``) survive; returns the same root instance when no
    references are present. Raises :class:`ValueError` if a referenced variable
    is not set.
    """
    return cast(Config, _resolve_in_place(config))


def _resolve_in_place(obj: Any) -> Any:
    if isinstance(obj, str):
        new_s = _ENV_REF_PATTERN.sub(_env_replace, obj)
        return new_s if new_s != obj else obj
