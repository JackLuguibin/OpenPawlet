...
from openpawlet.cli import commands
from openpawlet.cli import stream as stream_mod
...
def mock_prompt_session():
    ...
    with (
        patch("openpawlet.cli.commands._PROMPT_SESSION", mock_session),
        patch("openpawlet.cli.commands.patch_stdout"),
    ):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    with (
        patch("openpawlet.cli.commands.PromptSession") as MockSession,
        patch("openpawlet.cli.commands.FileHistory") as MockHistory,
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch("openpawlet.cli.commands._print_interactive_line", side_effect=fake_print):
        thinking = stream_mod.ThinkingSpinner(console=mock_console)
        with thinking:
...
def test_response_renderable_uses_text_for_explicit_plain_rendering():
    status = "🐈 openpawlet v0.1.4.post5\n🧠 Model: MiniMax-M2.7\n📊 Tokens: 20639 in / 29 out"
