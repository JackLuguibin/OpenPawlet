...
from openpawlet.cli.commands import _make_provider, app
...
# ``src/openpawlet/tests/console/test_unified_app.py``.
_SKIP_LEGACY_CLI = pytest.mark.skip(
    reason=(
        "legacy gateway / serve subcommands removed in "
...
def mock_paths():
    ...
    with (
        patch("openpawlet.config.loader.get_config_path") as mock_cp,
        patch("openpawlet.config.loader.save_config") as mock_sc,
        patch("openpawlet.config.loader.load_config") as mock_lc,
        patch("openpawlet.cli.commands.get_workspace_path") as mock_ws,
    ):
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "OpenPawlet is ready" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from openpawlet.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "openpawlet.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("openpawlet.channels.registry.discover_all", lambda: {})
...
def test_config_explicit_longcat_provider_uses_default_api_base():
    config = Config.model_validate(
        {
            "agents": {
                "defaults": {
...
def test_config_auto_detects_longcat_from_model_keyword():
    config = Config.model_validate(
        {
            "agents": {"defaults": {"provider": "auto", "model": "longcat/some-model"}},
...
def test_make_provider_uses_github_copilot_backend():
    from openpawlet.cli.commands import _make_provider
    from openpawlet.config.schema import Config
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("openpawlet.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
...
def mock_agent_runtime(tmp_path):
    ...
    with (
        patch("openpawlet.config.loader.load_config", return_value=config) as mock_load_config,
        patch("openpawlet.config.loader.resolve_config_env_vars", side_effect=lambda c: c),
        patch("openpawlet.cli.commands.sync_workspace_templates") as mock_sync_templates,
        patch("openpawlet.cli.commands._make_provider", return_value=object()),
        patch("openpawlet.cli.commands._print_agent_response") as mock_print_response,
        patch("openpawlet.bus.queue.MessageBus"),
        patch("openpawlet.cron.service.CronService"),
        patch("openpawlet.agent.loop.AgentLoop") as mock_agent_loop_cls,
    ):
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "openpawlet.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("openpawlet.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("openpawlet.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("openpawlet.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("openpawlet.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("openpawlet.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("openpawlet.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "openpawlet.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("openpawlet.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("openpawlet.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("openpawlet.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("openpawlet.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("openpawlet.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("openpawlet.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("openpawlet.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "openpawlet.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("openpawlet.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("openpawlet.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("openpawlet.config.paths.get_cron_dir", lambda: legacy_dir)
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("openpawlet.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("openpawlet.config.loader.resolve_config_env_vars", lambda c: c)
    monkeypatch.setattr(
        "openpawlet.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "openpawlet.cli.commands._make_provider",
        make_provider or (lambda _config: object()),
    )

    if message_bus is not None:
        monkeypatch.setattr("openpawlet.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("openpawlet.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("openpawlet.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("openpawlet.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    _patch_cli_command_runtime(
        ...
        session_manager=lambda *_a, **_k: object(),
    )
    monkeypatch.setattr("openpawlet.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("openpawlet.api.server.create_app", _fake_create_app)
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    monkeypatch.setattr("openpawlet.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("openpawlet.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("openpawlet.cli.commands._make_provider", lambda _config: provider)
    monkeypatch.setattr("openpawlet.bus.queue.MessageBus", lambda: bus)
    monkeypatch.setattr("openpawlet.session.manager.SessionManager", lambda *_a, **_k: object())
    ...
    monkeypatch.setattr("openpawlet.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("openpawlet.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("openpawlet.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from openpawlet.cli.commands import _migrate_cron_store
    ...
    with patch("openpawlet.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_health_endpoint_binds_and_serves_expected_responses(
    ...
    for _k in (
        "OPENPAWLET_TEAM_ID",
        "OPENPAWLET_TEAM_ROOM_ID",
        "OPENPAWLET_TEAM_MEMBER_IDS",
        "OPENPAWLET_TEAM_IN_PROCESS",
    ):
        monkeypatch.delenv(_k, raising=False)
    config_file = _write_instance_config(tmp_path)
    config = Config()
    # Isolate workspace so we do not infer OPENPAWLET_AGENT_ID from the developer's
    # ~/.openpawlet/workspace or trigger team run_team_member_event_loop in tests.
    ...
    monkeypatch.setattr("openpawlet.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("openpawlet.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("openpawlet.cron.service.CronService", _FakeCronService)
    monkeypatch.setattr("openpawlet.heartbeat.service.HeartbeatService", _FakeHeartbeatService)
