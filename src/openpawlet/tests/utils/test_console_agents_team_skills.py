...
from openpawlet.config.schema import AgentDefaults, AgentsConfig, Config
...
def _write_team(workspace: Path, team_id: str, team_skills: list[str]) -> None:
    p = workspace / ".openpawlet_console"
...
def test_team_skills_merges_disabled_with_defaults(tmp_path: Path) -> None:
    ...
    with patch(
        "openpawlet.utils.console_agents.disabled_skills_for_allowlist",
        return_value={"x1", "y1"},
    ) as m_deny:
...
def test_team_skills_intersects_with_agent_allowlist(
    ...
    with patch("openpawlet.utils.console_agents.disabled_skills_for_allowlist") as m_deny:
        m_deny.side_effect = [
            {"d1", "d2"},
            {"d3", "d4"},
        ]
