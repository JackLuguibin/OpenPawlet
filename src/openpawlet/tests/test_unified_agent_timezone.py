...
from openpawlet.utils.helpers import local_now, timestamp
from openpawlet.utils.token_usage_jsonl import TokenUsageJsonlRecorder
...
def test_memory_store_append_history_uses_store_timezone(
    ...
    monkeypatch.setattr("openpawlet.agent.memory.local_now", _spy)
...
def test_token_usage_jsonl_partitions_by_agent_calendar_day(tmp_path: Path) -> None:
    ...
    with patch("openpawlet.utils.token_usage_jsonl.local_now", return_value=fixed):
