"""Tests for the ask_user tool and its loop/runner integration."""
...
from openpawlet.agent.tools.ask import AskUserInterrupt, AskUserTool
from openpawlet.agent.tools.base import Tool, tool_parameters
from openpawlet.agent.tools.registry import ToolRegistry
from openpawlet.agent.tools.schema import tool_parameters_schema
from openpawlet.bus.events import InboundMessage
from openpawlet.bus.queue import MessageBus
from openpawlet.providers.base import GenerationSettings, LLMResponse, ToolCallRequest
...
async def test_ask_user_keeps_buttons_for_telegram(tmp_path):
    ...
    response = await loop._process_message(
        InboundMessage(
            channel="telegram", sender_id="user", chat_id="123", content="set it up"
        )
    )
