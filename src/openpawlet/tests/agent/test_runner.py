...
from openpawlet.providers.base import LLMResponse, ToolCallRequest
...
def _make_loop(tmp_path):
    ...
    with (
        ...
        patch("openpawlet.agent.loop.SubagentManager") as MockSubMgr,
    ):
...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / ".openpawlet" / "tool-results" / "test_runner" / "call_big.txt").exists()


def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    from openpawlet.utils.helpers import maybe_persist_tool_result

    root = tmp_path / ".openpawlet" / "tool-results"
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "openpawlet.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "openpawlet.utils.helpers.logger.warning",
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr(
        "openpawlet.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None)
    )
    token_sizes = {
        "old user": 120,
        "tool call": 120,
    ...
    monkeypatch.setattr(
        "openpawlet.agent.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch(
        "openpawlet.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")
    ):
        result = await runner.run(
            AgentRunSpec(
                initial_messages=[{"role": "user", "content": "do task"}],
...
async def test_llm_error_not_appended_to_session_messages():
    ...
    from openpawlet.agent.runner import (
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from openpawlet.agent.loop import AgentLoop
    from openpawlet.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from openpawlet.bus.events import InboundMessage
    from openpawlet.bus.queue import MessageBus
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("openpawlet.agent.tools.filesystem.ListDirTool.execute", fake_execute)
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from openpawlet.agent.runner import _MAX_LENGTH_RECOVERIES, AgentRunner, AgentRunSpec
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from openpawlet.agent.runner import _BACKFILL_CONTENT, AgentRunner
...
def test_drop_orphan_tool_results_removes_unmatched_tool_messages():
    from openpawlet.agent.runner import AgentRunner
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from openpawlet.agent.loop import AgentLoop
    from openpawlet.agent.runner import _BACKFILL_CONTENT
    from openpawlet.bus.events import InboundMessage
    from openpawlet.bus.queue import MessageBus
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from openpawlet.agent.runner import _BACKFILL_CONTENT, AgentRunner, AgentRunSpec
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from openpawlet.agent.runner import _MICROCOMPACT_KEEP_RECENT, AgentRunner
...
async def test_injection_cycles_capped_at_max():
    ...
    from openpawlet.agent.runner import _MAX_INJECTION_CYCLES, AgentRunner, AgentRunSpec
    from openpawlet.bus.events import InboundMessage
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from openpawlet.agent.loop import AgentLoop
    from openpawlet.agent.runner import _MAX_INJECTIONS_PER_TURN
    from openpawlet.bus.events import InboundMessage
    from openpawlet.bus.queue import MessageBus
...
async def test_drain_injections_on_empty_final_response():
    ...
    from openpawlet.agent.runner import _MAX_EMPTY_RETRIES, AgentRunner, AgentRunSpec
    from openpawlet.bus.events import InboundMessage
...
def test_snip_history_preserves_user_message_after_truncation(monkeypatch):
    ...
    messages = [
        ...
        {"role": "user", "content": ".openpawlet的同目录"},
        {
            "role": "assistant",
            "content": None,
            "tool_calls": [
    ...
    monkeypatch.setattr(
        "openpawlet.agent.runner.estimate_prompt_tokens_chain", lambda *_a, **_kw: (500, None)
    )
