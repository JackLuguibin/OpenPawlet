"""Tests for openpawlet.agent.skills.SkillsLoader."""
...
def _write_skill(
    ...
    *,
    ...
    """Create ``base / name / SKILL.md`` with optional OpenPawlet metadata JSON."""
    skill_dir = base / name
    skill_dir.mkdir(parents=True)
    lines = ["---"]
    if metadata_json is not None:
        payload = json.dumps({"openpawlet": metadata_json}, separators=(",", ":"))
...
def test_list_skills_filter_unavailable_excludes_unmet_bin_requirement(
    ...
    _write_skill(
        skills_root,
        "needs_bin",
        metadata_json={"requires": {"bins": ["openpawlet_test_fake_binary"]}},
    )
    builtin = tmp_path / "builtin"
    builtin.mkdir()

    def fake_which(cmd: str) -> str | None:
        if cmd == "openpawlet_test_fake_binary":
            return None
        return "/usr/bin/true"

    monkeypatch.setattr("openpawlet.agent.skills.shutil.which", fake_which)
...
def test_list_skills_filter_unavailable_includes_when_bin_requirement_met(
    ...
    def fake_which(cmd: str) -> str | None:
        if cmd == "openpawlet_test_fake_binary":
            return "/fake/openpawlet_test_fake_binary"
        return None
...
def test_list_skills_filter_unavailable_false_keeps_unmet_requirements(
    ...
    monkeypatch.setattr("openpawlet.agent.skills.shutil.which", lambda _cmd: None)
...
def test_list_skills_filter_unavailable_excludes_unmet_env_requirement(
    ...
    _write_skill(
        skills_root,
        "needs_env",
        metadata_json={"requires": {"env": ["OPENPAWLET_SKILLS_TEST_ENV_VAR"]}},
    )
    builtin = tmp_path / "builtin"
    builtin.mkdir()

    monkeypatch.delenv("OPENPAWLET_SKILLS_TEST_ENV_VAR", raising=False)
...
def test_get_skill_metadata_handles_yaml_types(tmp_path: Path) -> None:
    ...
    skill_dir.mkdir(parents=True)
    payload = json.dumps(
        {"openpawlet": {"requires": {"bins": ["gh"]}, "always": True}}, separators=(",", ":")
    )
    skill_path = skill_dir / "SKILL.md"
    skill_path.write_text(
        f"---\nname: typed\nmetadata: {payload}\nalways: true\n---\n\n# Typed\n",
