...
def test_bootstrap_files_are_backed_by_templates() -> None:
    template_dir = pkg_files("openpawlet") / "templates"
...
def test_recent_history_truncated_at_max_chars(tmp_path) -> None:
    ...
    even when a single history entry would exceed the cap on its own."""
    workspace = _make_workspace(tmp_path)
    builder = ContextBuilder(workspace)
    ...
    prompt = builder.build_system_prompt()
    parts = prompt.split("# Recent History\n\n", 1)
    assert len(parts) == 2
    assert len(parts[1]) < builder._MAX_HISTORY_CHARS + 200
...
def test_identity_has_no_behavioral_instructions(tmp_path) -> None:
    ...
    assert "You are OpenPawlet" not in identity
...
def test_default_soul_template_contains_execution_rules() -> None:
    ...
    soul = (pkg_files("openpawlet") / "templates" / "SOUL.md").read_text(encoding="utf-8")
...
def test_template_memory_md_is_skipped(tmp_path) -> None:
    ...
    assert "This file is automatically updated by OpenPawlet" not in prompt
