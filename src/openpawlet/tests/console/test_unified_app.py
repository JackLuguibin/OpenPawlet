...
the ``/openpawlet-ws/*`` reverse-proxy and the SPA fallback in a single
process.

The embedded OpenPawlet runtime is disabled via ``OPENPAWLET_DISABLE_EMBEDDED``
...
def test_lifespan_brings_up_and_tears_down_subsystems(
    ...
    """A fake EmbeddedOpenPawlet must see start() then stop() during lifespan."""
    ...
    import openpawlet.runtime.embedded as embedded_mod

    monkeypatch.setattr(embedded_mod, "EmbeddedOpenPawlet", _FakeEmbedded)
...
def test_zmq_module_has_been_removed() -> None:
    """Importing ``openpawlet.bus.zmq_bus`` must fail in the consolidated layout."""
    import importlib

    with pytest.raises(ModuleNotFoundError):
        importlib.import_module("openpawlet.bus.zmq_bus")
