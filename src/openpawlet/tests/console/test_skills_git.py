...
def workspace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated workspace + OpenPawlet ``config.json`` for the router under test.
    ...
    ``~/.openpawlet`` (or compatibility fallback directories) directory.
    """
    work = tmp_path / "work"
    work.mkdir()
    cfg_path = tmp_path / "openpawlet_config.json"
    cfg_path.write_text(
