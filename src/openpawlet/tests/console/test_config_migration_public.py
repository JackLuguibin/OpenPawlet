"""Verify that ``migrate_config`` is a stable public symbol on openpawlet.

The console layer reads ``~/.openpawlet/config.json`` via this helper; we need
...
from openpawlet.config.loader import _migrate_config, migrate_config
