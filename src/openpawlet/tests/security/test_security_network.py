"""Tests for openpawlet.security.network — SSRF protection and internal URL detection."""
...
from openpawlet.security.network import (
...
def test_blocks_private_ipv4(ip: str, label: str):
    with patch("openpawlet.security.network.socket.getaddrinfo", _fake_resolve("evil.com", [ip])):
...
def test_blocks_ipv6_loopback():
    ...
    with patch("openpawlet.security.network.socket.getaddrinfo", _resolver):
...
def test_allows_public_ip():
    with patch(
        "openpawlet.security.network.socket.getaddrinfo",
...
def test_allows_normal_https():
    with patch(
        "openpawlet.security.network.socket.getaddrinfo", _fake_resolve("github.com", ["140.82.121.3"])
    ):
        ok, err = validate_url_target("https://github.com/JackLuguibin/OpenPawlet")
        assert ok
...
def test_detects_wget_localhost():
    with patch(
        "openpawlet.security.network.socket.getaddrinfo", _fake_resolve("localhost", ["127.0.0.1"])
    ):
...
def test_blocks_cgnat_by_default():
    ...
    with patch(
        "openpawlet.security.network.socket.getaddrinfo", _fake_resolve("ts.local", ["100.100.1.1"])
    ):
...
def test_whitelist_does_not_affect_other_blocked():
    ...
    try:
        with patch(
            "openpawlet.security.network.socket.getaddrinfo", _fake_resolve("evil.com", ["10.0.0.1"])
        ):
