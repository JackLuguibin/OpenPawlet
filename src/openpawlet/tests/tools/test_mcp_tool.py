...
import openpawlet.agent.tools.mcp as mcp_mod
...
from openpawlet.config.schema import MCPServerConfig
...
async def test_tool_does_not_retry_on_cancelled_error() -> None:
    ...
    with patch("openpawlet.agent.tools.mcp.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
...
async def test_connect_mcp_servers_enabled_tools_warns_on_unknown_entries(
    ...
    monkeypatch.setattr("openpawlet.agent.tools.mcp.logger.warning", _warning)
...
async def test_connect_mcp_servers_logs_stdio_pollution_hint(
    ...
    monkeypatch.setattr("openpawlet.agent.tools.mcp.logger.error", _error)
