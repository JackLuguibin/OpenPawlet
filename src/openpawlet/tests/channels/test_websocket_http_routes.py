"""HTTP routes on the WebSocket channel (session API, localhost bootstrap)."""
...
async def test_bootstrap_returns_token_for_localhost(bus: MagicMock, tmp_path: Path) -> None:
    ...
    try:
        resp = await _http_get("http://127.0.0.1:29921/api/bootstrap")
...
async def test_sessions_routes_require_bearer_token(bus: MagicMock, tmp_path: Path) -> None:
    ...
    await asyncio.sleep(0.3)
    try:
        deny = await _http_get("http://127.0.0.1:29922/api/sessions")
        assert deny.status_code == 401

        boot = await _http_get("http://127.0.0.1:29922/api/bootstrap")
        token = boot.json()["token"]
        auth = {"Authorization": f"Bearer {token}"}

        listing = await _http_get("http://127.0.0.1:29922/api/sessions", headers=auth)
        ...
        msgs = await _http_get(
            "http://127.0.0.1:29922/api/sessions/websocket:abc/messages",
            headers=auth,
        )
...
async def test_sessions_list_only_returns_websocket_sessions_by_default(
    ...
    await asyncio.sleep(0.3)
    try:
        boot = await _http_get("http://127.0.0.1:29923/api/bootstrap")
        token = boot.json()["token"]
        auth = {"Authorization": f"Bearer {token}"}
...
async def test_session_delete_removes_file(bus: MagicMock, tmp_path: Path) -> None:
    ...
    channel = _ch(bus, session_manager=sm, port=29924)
    server_task = asyncio.create_task(channel.start())
    await asyncio.sleep(0.3)
    try:
        boot = await _http_get("http://127.0.0.1:29924/api/bootstrap")
        token = boot.json()["token"]
        auth = {"Authorization": f"Bearer {token}"}
        ...
        resp = await _http_get(
            "http://127.0.0.1:29924/api/sessions/websocket:doomed/delete",
            headers=auth,
        )
        assert resp.status_code == 200
...
async def test_session_routes_accept_percent_encoded_websocket_keys(
    ...
    await asyncio.sleep(0.3)
    try:
        boot = await _http_get("http://127.0.0.1:29925/api/bootstrap")
        token = boot.json()["token"]
        auth = {"Authorization": f"Bearer {token}"}

        msgs = await _http_get(
            "http://127.0.0.1:29925/api/sessions/websocket%3Aencoded-key/messages",
            headers=auth,
        )
        assert msgs.status_code == 200
        ...
        deleted = await _http_get(
            "http://127.0.0.1:29925/api/sessions/websocket%3Aencoded-key/delete",
            headers=auth,
        )
...
async def test_session_routes_reject_non_websocket_keys(bus: MagicMock, tmp_path: Path) -> None:
    ...
    await asyncio.sleep(0.3)
    try:
        boot = await _http_get("http://127.0.0.1:29926/api/bootstrap")
        token = boot.json()["token"]
        auth = {"Authorization": f"Bearer {token}"}

        msgs = await _http_get(
            "http://127.0.0.1:29926/api/sessions/cli:direct/messages",
            headers=auth,
        )
        ...
        deny_delete = await _http_get(
            "http://127.0.0.1:29926/api/sessions/slack:C123/delete",
            headers=auth,
        )
...
async def test_session_routes_reject_invalid_key(bus: MagicMock, tmp_path: Path) -> None:
    ...
    await asyncio.sleep(0.3)
    try:
        boot = await _http_get("http://127.0.0.1:29927/api/bootstrap")
        token = boot.json()["token"]
        auth = {"Authorization": f"Bearer {token}"}

        resp = await _http_get(
            "http://127.0.0.1:29927/api/sessions/bad%20key/messages",
            headers=auth,
        )
...
async def test_unknown_route_returns_404(bus: MagicMock) -> None:
    ...
    await asyncio.sleep(0.3)
    try:
        resp = await _http_get("http://127.0.0.1:29928/api/unknown")
