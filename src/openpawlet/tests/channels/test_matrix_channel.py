...
async def test_start_skips_load_store_when_device_id_missing(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("openpawlet.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "openpawlet.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("openpawlet.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr("openpawlet.channels.matrix.asyncio.create_task", _fake_create_task)
...
def test_build_matrix_text_content_basic_text() -> None:
    ...
    expected = {"msgtype": "m.text", "body": "Hello, World!", "m.mentions": {}}
