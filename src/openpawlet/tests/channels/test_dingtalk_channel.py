...
try:
    from openpawlet.channels import dingtalk
...
import openpawlet.channels.dingtalk as dingtalk_module
from openpawlet.bus.queue import MessageBus
from openpawlet.channels.dingtalk import DingTalkChannel, DingTalkConfig, OpenPawletDingTalkHandler
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "OpenPawlet Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = OpenPawletDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/openpawlet_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/openpawlet_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "openpawlet.config.paths.get_media_dir",
