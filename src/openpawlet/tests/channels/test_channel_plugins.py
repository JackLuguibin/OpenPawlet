...
from openpawlet.config.schema import ChannelsConfig
from openpawlet.utils.restart import RestartNotice
...
def test_discover_plugins_loads_entry_points():
    from openpawlet.channels.registry import discover_plugins
...
async def test_manager_loads_plugin_from_dict_config():
    ...
    with patch(
        "openpawlet.channels.registry.discover_all",
...
async def test_base_channel_passes_api_base_to_openai_transcription_provider():
    ...
    from openpawlet.providers import transcription as transcription_mod
...
def test_openai_transcription_provider_honors_api_base_argument():
    from openpawlet.providers.transcription import OpenAITranscriptionProvider
...
from openpawlet.providers.transcription import GroqTranscriptionProvider as _GroqProvider
from openpawlet.providers.transcription import OpenAITranscriptionProvider as _OpenAIProvider
...
async def test_transcription_provider_includes_language(tmp_path, provider_cls, language):
    ...
    with patch(
        "openpawlet.providers.transcription.httpx.AsyncClient",
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    monkeypatch.setattr("openpawlet.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "openpawlet.channels.registry.discover_all",
...
def test_builtin_channel_default_config():
    ...
    from openpawlet.channels.telegram import TelegramChannel
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("openpawlet.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("openpawlet.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("openpawlet.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
