...
def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("openpawlet.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
    ...
    assert headers["HTTP-Referer"] == "https://github.com/JackLuguibin/OpenPawlet"
    assert headers["X-OpenRouter-Title"] == "OpenPawlet"
...
def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    with patch("openpawlet.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
        OpenAICompatProvider(
            ...
            default_model="anthropic/claude-sonnet-4-5",
            extra_headers={
                "HTTP-Referer": "https://github.com/JackLuguibin/OpenPawlet",
                "X-OpenRouter-Title": "OpenPawlet Pro",
    ...
    headers = MockClient.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://github.com/JackLuguibin/OpenPawlet"
    assert headers["X-OpenRouter-Title"] == "OpenPawlet Pro"
    assert headers["X-OpenRouter-Categories"] == "cli-agent,personal-agent"
...
async def test_openai_compat_stream_watchdog_returns_error_on_stall(monkeypatch) -> None:
    monkeypatch.setenv("OPENPAWLET_STREAM_IDLE_TIMEOUT_S", "0")
...
def test_dashscope_thinking_disabled_for_minimal() -> None:
    """'minimal' → wire 'minimum' + thinking off. DashScope rejects literal
    'minimal' (invalid_value); outbound translation preserves no-thinking via
    extra_body."""
...
def test_dashscope_thinking_disabled_for_minimum_alias() -> None:
    """Native 'minimum' (DashScope docs) is the same semantic as 'minimal';
    thinking must stay disabled, not enabled."""
...
def test_non_dashscope_minimal_not_retranslated() -> None:
    """DashScope-only wire mapping must not change 'minimal' for other providers."""
