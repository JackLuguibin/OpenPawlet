"""High-level programmatic interface to OpenPawlet."""
...
def _resolve_config(
    ...
    """Load + env-resolve an OpenPawlet ``Config`` from disk."""
...
class OpenPawlet:
    """Programmatic facade for running the OpenPawlet agent.
    ...
    def from_config(
        ...
        *,
        ...
        """Create an OpenPawlet instance from a config file.

        Args:
            config_path: Path to ``config.json``.  Defaults to
                ``~/.openpawlet/config.json`` (older default paths are still accepted when the preferred file is absent).
...
def _make_provider(config: Any) -> Any:
    """Backwards-compat shim around :func:`openpawlet.providers.factory.build_provider`."""
    from openpawlet.providers.factory import build_provider
