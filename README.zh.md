...
- **典型场景**：执行 `console start` 即可；内嵌的 nanobot 运行时随同一个 FastAPI 进程启动，无需再额外维护 gateway 进程，可以直接在控制台观察状态、调试会话、管理 Bot 相关资源。
...
> `/v1/*`、队列管理 `/queues/*`、WebSocket `/nanobot-ws/*` 与内嵌的 nanobot 运行时
> （AgentLoop、Channels、Cron、Heartbeat）共享同一事件循环、对外只暴露**一个 HTTP 端口**。
> 旧的 `nanobot gateway` / `nanobot serve` / `open-pawlet-queue-manager` /
...
使前端页面与 `/api/v1/*` 共用同一端口；**同时在同一事件循环里启动 nanobot 运行时**
（不再 fork 任何子进程，亦不再依赖 ZeroMQ broker），所有 WebSocket / 通道 / 定时任务
都在 lifespan 中托管。首次运行若缺少 `~/.nanobot/config.json` 请先执行 `nanobot onboard`。
按 Ctrl+C 优雅停止整个进程。
...
```text
2026-04-26 ──●── 0.3.0  单 FastAPI 服务收紧：合并 console + nanobot OpenAI API + queue admin + gateway 到单进程；移除 ZMQ broker / Procfile / honcho；in-process MessageBus；Win/Linux 兼容统一；新增 unified app 全链路测试
              │
...
| 日期 | 版本 | 摘要 |
|------|------|------|
| 2026-04-26 | **0.3.0** | **架构收紧：** 全部服务（REST / SPA / OpenAI 兼容 `/v1/*` / queues admin / WebSocket / nanobot 运行时）合并到单一 FastAPI 进程；移除 `nanobot gateway`、`nanobot serve`、`open-pawlet-queue-manager`、`Procfile` 与 ZeroMQ broker；in-process MessageBus 取代 ZMQ；统一 Win/Linux 事件循环与信号处理；新增 `EmbeddedNanobot` 运行时与 unified app 全链路测试。 |
